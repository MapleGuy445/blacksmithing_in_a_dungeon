...
public sealed class BlacksmithController : Component
{
	...
	protected override void OnPreRender()
	{
		if ( Carrying.IsValid() && !Carrying.IsProxy )
		{
			...
			var pc = Components.Get<BlacksmithController>();
	...
	void UpdatePickup()
	{
		...
		if ( Input.Pressed( "use" ) )
		{
			...
			if ( Carrying.IsValid() )
			{
				...
				{
					rb.Enabled = true;
					rb.Velocity = lookDir.Forward * 50.0f + Vector3.Up * 50.0f;
				}
	...
	void TryPickup()
	{
		...
		var tr = Scene.Trace.WithTag( "pickup" ).Sphere( 16, eyePos, eyePos + lookDir.Forward * 100 ).Run();
		if ( !tr.Hit ) return;
		...
		ca.HoldType = Sandbox.Citizen.CitizenAnimationHelper.HoldTypes.HoldItem;
	}
	...
	void Drop()
	{
		...
		//ca.IkLeftHand = null;
		//ca.IkRightHand = null;
		ca.HoldType = Sandbox.Citizen.CitizenAnimationHelper.HoldTypes.None;
	}
}